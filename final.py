...
# ==========================
# STEP 1: Split Dataset into 70/15/15 (UNCOMMENT IF NEEDED)
# ==========================
...
checkpoint = keras.callbacks.ModelCheckpoint(
    filepath=f"saved_models/best_animal_classifier_{timestamp}.h5",
    monitor="val_loss",
    save_best_only=True,
    mode="min",
    verbose=1
...
# ==========================
final_h5_path = f"saved_models/final_animal_classifier_{timestamp}.h5"
final_tf_path = f"saved_models/final_animal_classifier_{timestamp}_SavedModel"

model.save(final_h5_path)  # HDF5
model.save(final_tf_path, save_format="tf")  # TensorFlow SavedModel

print(f"Final model saved at:\n - {final_h5_path}\n - {final_tf_path}/")

# ==========================
# Auto-Cleanup (Keep only latest N runs)
# ==========================
def cleanup_saved_models(directory="saved_models", keep_latest=3):
    """Keep only the latest N saved model runs."""
    
    # Get all .h5 files sorted by modification time (newest first)
    h5_files = sorted(
    ...
    # Get all SavedModel folders sorted by modification time (newest first)
    sm_folders = sorted(
        glob.glob(os.path.join(directory, "*_SavedModel")),
        key=os.path.getmtime,
        reverse=True
    )
    
    # Delete old .h5 files
    if len(h5_files) > keep_latest:
        for h5_path in h5_files[keep_latest:]:
            if os.path.exists(h5_path):
                os.remove(h5_path)
                print(f"Deleted old model: {h5_path}")
    
    # Delete old SavedModel folders
    if len(sm_folders) > keep_latest:
        for sm_folder in sm_folders[keep_latest:]:
            if os.path.exists(sm_folder):
                shutil.rmtree(sm_folder)
                print(f"Deleted old SavedModel folder: {sm_folder}")

# Run cleanup
cleanup_saved_models("saved_models", keep_latest=3)

# modify your code to include fine-tuning and LR scheduling (for better accuracy)